...
abstract class AbstrState implements State {
  ...
  public void smallBlind(Player currentPlayer, int minimum, int amount, Pots pot)
    ...
    if (amount != Match.SMALLBLIND) {
      throw new GameException(
          "Small Blind doit etre de " + Match.SMALLBLIND + " different de " + amount);
    }
  ...
  public void allIn(Player currentPlayer, int minimum, Pots pot) throws GameException {
    if (currentPlayer.getMoney() >= minimum) {
      throw new GameException("Il vous reste suffisament d'argent pour parier "
          + currentPlayer.getMoney() + " " + minimum);
    }
    currentPlayer.allIn();
    nextState();
  }
