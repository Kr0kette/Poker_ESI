...
import esi.atl.g39871.poker.model.cards.ValueComparator;
...
/**
 * To build the best player's hand, the factory needs to build all the ways to chose a 5 cards in 7.
 * Each way forms a proposal
 * <p>
 */
class Proposal {
  ...
  /**
   * A proposal is build based on 2 cards from the player's cards and 3 cards from the board. During
   * this build the sum of the card's value, the amount of each color and the amount of each Value
   * are computed.
   * <p>
   */
  ...
  private Map<Value, Long> getMapValue() {
    Map<Value, Long> map =
        cards.stream().collect(Collectors.groupingBy(Card::getValue, Collectors.counting()));
  ...
  private Map<Color, Long> getMapColor() {
    Map<Color, Long> map =
        cards.stream().collect(Collectors.groupingBy(Card::getColor, Collectors.counting()));
    map.values().removeAll(Collections.singleton((long) 1));
    return map;
  }
  ...
  boolean isFull() {
    return mapValue.values().contains((long) 3) && mapValue.values().contains((long) 2)
        && mapValue.size() == 2;
  }
